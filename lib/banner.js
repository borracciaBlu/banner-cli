 ...
 * @return function
 */
function doAppendBanner(options, bannerText, logger) {
    return function(path) {
        ...
        files.map((file) => {
            ...
            !options.dryRun
                && prependFile.sync(file, bannerText);
        });
    };
}
 ...
 * @return void
 */
function banner(args, options, logger) {
    options = getOptions(options, pkg);

    let template = getTemplate(options);
    let bannerText = getBanner(template, options);

    logger.addBanner(bannerText);

    if (!args || args.length === 0) {
        throw new Error('File not found');
    }

    args.forEach(doAppendBanner(options, bannerText, logger));
    logger.completeBanner(bannerText);
}
